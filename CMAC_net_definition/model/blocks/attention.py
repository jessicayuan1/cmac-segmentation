...
class SpatialPSA(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(b, 1, -1)                          # σ₁ → (B, 1, C/2)
        q = self.softmax(q)                           # channel softmax

        # Value branch
        v = self.wv(x).view(b, -1, h * w)             # σ₂ → (B, C/2, HW)

        # Polarization
        attn = torch.matmul(q, v)                     # (B, 1, HW)
        attn = attn.view(b, 1, h, w)                   # σ₃
...
class PolarizedSelfAttention(nn.Module):
    ...
    def forward(self, x):
        z_ch = self.channel_psa(x)     # Ach(X) ⊙ch X
        z_sp = self.spatial_psa(z_ch) # Asp(Zch) ⊙sp Zch
        return z_sp
